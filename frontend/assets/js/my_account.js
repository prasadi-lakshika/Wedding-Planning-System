...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('linkGoogleBtn').addEventListener('click', () => {
    ...
    window.location.href = 'http://localhost:5000/auth/link/google';
  });

  document.getElementById('unlinkGoogleBtn').addEventListener('click', async () => {
    try {
      const response = await fetch('http://localhost:5000/api/unlink/google', { method: 'POST' });
      const result = await response.json();
      if (response.ok) {
  ...
  document.getElementById('linkFacebookBtn').addEventListener('click', () => {
    ...
    window.location.href = 'http://localhost:5000/auth/link/facebook';
  });

  document.getElementById('unlinkFacebookBtn').addEventListener('click', async () => {
    try {
      const response = await fetch('http://localhost:5000/api/unlink/facebook', { method: 'POST' });
      const result = await response.json();
      if (response.ok) {
        socialMsg.style.color = 'green';
